import { LutTypes, BaseLut } from './base';
...
export const WindowingLut = {
	match(lut: BaseLut): lut is WindowingLut { return lut.type === LutTypes.WINDOWING; },
	isCompatible(image: Image) { return image.components === 1; }
};
...
export const ThresholdWindowingLutRenderer2D: LutRenderer2D = {
	match(lut: BaseLut): lut is ThresholdWindowingLut {
		return lut && (<ThresholdWindowingLut>lut).center !== undefined && (<ThresholdWindowingLut>lut).width === 1;
	},
